...
import {PackageInfo, PluginInfo} from './types'
...
async function run(): Promise<void> {
  try {
    ...
    try {
      const pluginInfoData = fs.readFileSync(
    ...
    if (pluginId === '') {
      ...
      if (pluginId === '') {
        throw new Error(
          '`package_id` in package.json and the `plugin_id` argument must not both be empty'
        )
      }
    }
    ...
    for (const majorVersion of majorSDKVersions) {
      const semanticVersion = `${majorVersion}.0.0`
      let curaVersions = ''
      switch (majorVersion) {
        case 5:
          curaVersions = '3.5-3.6'
          break

        case 6:
          curaVersions = '4.0-4.3'
          break

        case 7:
          curaVersions = '4.4-4.13'
          break
      ...
      archive.directory(sourcePath, `files/plugins/${pluginId}`, file =>
        file.name.startsWith('.git') || file.name.includes('/.git')
          ? false
          : file
      )
